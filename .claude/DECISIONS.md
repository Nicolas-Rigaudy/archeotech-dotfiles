...
### [2026-07-30] Migrated mangowc → mangowm 0.15; mmsg IPC is now JSON

The AUR package `mangowc-git` was renamed **`mangowm`** upstream (same binary `mango`, same `~/.config/mango`). Updated to `mangowm 0.15.5` mainly to pull in **wlroots 0.20** for the dock-undock output-hotplug freeze (see ROADMAP QA). The update also **rewrote the `mmsg` IPC**: the old flags (`mmsg -O`/`-w`/`-s -d`/`-g -k`) were removed for subcommands — `mmsg get <type>` / `watch <type>` / `dispatch <func>` — emitting **JSON**. That broke every consumer on the old flags; ported all three: `MangoWC.qml` (two JSON watch streams — `all-monitors` + `focusing-client` — replacing the single `-w -O -t…` text stream, same public API), `mango-reload.sh`, and `theme-switch.py` (`_kb_layout` / `mango_reload_preserving_kb`). Supersedes the transport detail of [2026-05-04] "MangoWC IPC: mmsg -w subprocess stream" — the streaming approach stands; only the CLI + wire format changed. Hard-won: `pkill -x mango` kills the *real* session compositor, so nested-compositor teardown must kill by captured PID (memory `never-kill-mango-by-name`).

### [2026-07-30] Hyprland promoted to a pre-v1.0 target (dual setup) — partially revises the multi-compositor defer

Pulled **Hyprland** support + the `CompositorService` facade forward from post-v1.0 to pre-v1.0 (ROADMAP Sprint 29), partially revising [2026-07-01]'s "portability is post-v1.0." Three new inputs: (1) **audience** — Quickshell is compositor-agnostic and almost nobody runs MangoWC, so a public shell that only runs on mango has near-zero addressable market → Hyprland is table stakes for "anyone can install it," not depth; (2) **dogfooding** — running daily on both compositors is the only forcing function that makes the (still unbuilt) facade real rather than theoretical; (3) a **MangoWC dock-undock freeze** (wlroots output-hotplug hang, diagnosed 2026-07-30) makes Hyprland a needed stability fallback on this docking laptop. Scope held tight: **Hyprland only** — Niri/Sway stay post-v1.0. The [2026-07-01] extensibility-first logic otherwise stands; this moves only the Hyprland slice. Trade-off: one more sprint before the v1.0 tag, in exchange for a shell people can actually run. Cost is bounded — coupling is shallow (11 `mmsg` sites across 9 files) and the built-in `Quickshell.Hyprland` service does much of the workspace/window/event work mango needed a custom `mmsg -w` stream for. Evolves [2025-12] "Hyprland fallback" → Hyprland is now a co-equal *project target* (mango stays the daily-driver primary for now).
...
### [2026-07-16] Picker redesign gotchas: `file://` URL paths, hot-reload vs restart, image decode

Three facts that cost real debugging on the wallpaper picker (full study: ANALYSIS §19). **(1)** `StandardPaths.writableLocation` (hence `Commons.Paths.home`/`.cache`/`.config`) returns a **`file://` URL, not a plain path** — so `"file://" + (Paths.cache + "/x")` produces `file://file///…` and the image silently fails. For a filesystem `Image.source`, strip it: `Paths.cache.replace(/^file:\/\//,"")` then re-prefix once. This bug made *every* wallpaper thumbnail fail → fall back to decoding the full 4K/6K original, which looked like "thumbnails are just slow" for several iterations (the fallback masked it). Paths from `find`/`Process` stdout are already plain — the mismatch is the trap. **(2)** QML **hot-reload does not reliably re-run singletons / `Component.onCompleted` one-time scans** — new singletons + rescans need a **full shell restart**. So `mango-reload.sh` (Super+Shift+R) *must* restart Quickshell (`qs -c archeotech kill` → relaunch `quickshell -c archeotech` LAST, after `wlr-randr` re-applies monitors); a "reload config only, keep the shell" variant breaks the reload-to-test workflow. **(3)** `asynchronous: false` on an `Image` moves decode onto the **open animation** (janky open) — keep async; make thumbnails tiny + the model/view persistent instead. Also: `PathView` has **no `positionViewAtIndex`** — set `currentIndex` (auto-centres under `StrictlyEnforceRange` + `preferredHighlightBegin: 0.5`).
...
Non-obvious decisions made during execution: (1) **kitty theme confs are theme assets, not personal kitty prefs** — moved from `~/.config/kitty/themes/<name>.conf` into each theme package as `themes/<variant>/kitty.conf`; `apply_kitty` reads the co-located file (via a new `theme["_dir"]`), so a theme is self-contained and the kitty-config-dir coupling is gone. (2) **The runtime `~/.config/archeotech/wallpapers` stays a symlink to the *personal* set on this machine** while the public `install.sh` only seeds 2 non-IP defaults *if absent* — so my full 42M set isn't shipped and a stranger still gets a working picker; `find -L` in the picker follows the symlinked dir. (3) **Single launcher** — Quickshell does **not** dedupe per config, so two launch points (`config.conf` `exec-once` + `autostart.sh`) would double the bars + exclusion zones; kept `exec-once` as authoritative, disabled the `autostart.sh` line. (4) **Config paths must resolve relative to the shell dir under `qs -c <name>`** — the move surfaced a real bug: `ShellConfig` hardcoded `~/.config/quickshell/shell-config.json` and `ModuleRegistry` hardcoded `~/.config/quickshell/modules` (both correct only when the shell *is* the default config). Under `-c archeotech` those paths don't exist, so the shell silently ran on built-in `_defaults` and bundled modules weren't discovered. This also explained a "strip popups look smaller" symptom the user flagged — it was the shell on defaults, **not** the `PanelHost` `Math.min(screenAxis,…)` screen-clamp I first guessed (lesson: check whether config even loads before reasoning from the sizing code). Fix: `shell-config.json` now lives in `~/.config/archeotech/` (per-user + writable, exactly like `theme.json`/`config.json`); the repo ships a **default** that `install.sh` copies there if absent (so edit-mode edits never dirty the repo clone), and `ModuleRegistry` resolves its bundled dir via `Qt.resolvedUrl` relative to the shell root. Trade-off accepted: a cross-repo runtime coupling remains (e.g. `battery-alert.service` lives in private but `ExecStart`s the now-public `~/.local/bin/battery-alert.sh`) — fine on this machine, and the service is machine-level while the script is shell-integral.

### [2026-07-31] Tiling-layout picker + layout-UX pass; reload preserves layout by dropping tagrules; click-outside hits the card rect

Built the ROADMAP "tiling-layout picker" — a `PanelRegistry` panel (`Widgets/Appearance/LayoutPickerBody.qml` + `Modules/Shell/Panels/Content/LayoutPicker.qml`, id `layout`, `Super+Shift+T`) showing all 14 mango layouts as **static Rectangle mini-diagrams** driven by one JS recipe table (no per-layout QML), mauve master accent, per-layout keybind chip (doubles as a quicksheet) + hover description, click / arrows+Enter to `MangoWC.dispatch("setlayout <name>")`. First real use of the `shot.sh` headless harness (verified render without touching the live session).

Load-bearing decisions & gotchas (each cost real work):
- **A panel needs an opener host or it silently no-ops.** `openGlobal` sets state, but `Strip._showsPanel(id)` renders only if `id` is in a side's `content` (`stripIcons` = raw entry ids). So a new panel needs (1) `PanelRegistry` entry, (2) `shell.qml` `IpcHandler`, **and (3)** a registered opener in `WidgetRegistry.availableWidgets` (+ `_panelOpenerIds`) listed in `shell-config.json` content. Missing (3) → IPC returns 0, nothing shows. Memory: `shell-panel-needs-opener-host`.
- **mmsg JSON has no layout *name*** — only `layout_symbol` (terse `T`/`S`…) + `layout_index`. So the picker's active-highlight is authoritative from what it last set, seeded best-effort from a symbol→name map (only `T`/`S` confirmed live). `setlayout` takes a name; there is no set-by-index.
- **Reload was resetting the layout** (ugly full-width "stretch"): `reload_config` re-applied the per-tag `layout_name:scroller` tagrules. First tried capturing+restoring via `switch_layout` cycling in `mango-reload.sh`, but the reset still animated. **Fix: dropped the tagrules entirely** (user doesn't use per-tag layouts) so reload preserves the live layout on its own — and removed the restore hack. Trade-off accepted: a fresh/empty tag now defaults to mango's built-in **tile** (there is no global default-layout config), not scroller; `Super+Space`/picker gets scroller.
- **Click-outside-to-dismiss was shell-wide broken for compact panels:** it hit-tested the full-edge `SideLoader` rect, so the band beside (or above/below, for vertical) a narrower centred panel counted as "inside." Fixed by exposing the actual `card` rect from `Strip` (`panelRect`, window coords) and testing that in `ShellSurface`'s handler when a panel is open — fixes every strip panel, not just the picker.
- **Keyboard-focus vs Esc:** giving the grid `activeFocus` (for arrow nav) meant it swallowed Escape before the Strip's `Keys.onEscapePressed` — so the content handles Esc itself (`panelRoot.close()`), mirroring how the Launcher's TextField coexists with the Strip.

Also bundled: `circle_layout` extended to all 14 (Super+T cycles everything), and new mango binds — `Super+Return`=zoom(→master), `Super+J`/`Shift+J`=focusstack, `Super+Z`=focuslast, `Super+A`=toggleoverview (note `Alt+Tab`=togglejump already existed), `Super+Alt+R`=right_tile, `Super+Alt+C`=center_tile. Research + design in `.claude/TILING-PICKER-RESEARCH.md`.
