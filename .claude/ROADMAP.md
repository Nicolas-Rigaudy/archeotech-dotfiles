...
| 24 (part 1) | Settings/IA restructure — dissolved Control Center (deleted, ~1035 LOC); Settings → unified right-strip panel (was FloatingWindow); standalone Media panel (bottom); DND → Notifications header; theme+wallpaper+logo unified via shared `Widgets/Appearance/{ThemeGridBody,WallpaperPickerBody}`, hosted by Settings→Appearance + bottom Appearance switcher; triggers/deep-links rewired. Depth tasks (below) still pending. | 2026-06-11 |
