...
**Last Updated:** 2026-06-03 (Sprint 21 Chunks 0+1 — `ShellConfig` write-back (`setSideType`/`setZoneWidgets`/`setStripIcons`, deep-clone→rewrite, `$schema` preserved); visual builder edit mode (`Modules/Shell/Builder/EditOverlay.qml` + `WidgetPalette.qml`, `editmode` IPC, `Super+Shift+E`) — click-to-assign bar zones / strip icons + remove/reorder, reads config & renders an abstract map (live shell hot-reloads underneath); new `holder` side type (hidden-at-rest hover-reveal `Strip` in holderMode, gap-0, flush popups); `WidgetRegistry.availableBarWidgets`/`availableStripIcons` palette seam. Chunks 2 (module manifest) + 3 (desktop widgets) remain)
...
**Next Sprint:** Sprint 21 continues — Chunk 2 (`module.json` manifest + `ModuleRegistry`, `~/.local/share/archeotech/modules/`) + Chunk 3 (desktop widget layer on `WlrLayer.Bottom` + peek-desktop access). Then Sprint 22 — Adaptive Shell Frame (concave-blend ↔ convex-cap corners per adjacency + isolated-bar pill toggle).
