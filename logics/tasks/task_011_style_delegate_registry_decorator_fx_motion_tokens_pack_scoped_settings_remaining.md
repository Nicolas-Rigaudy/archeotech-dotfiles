...
- Wave 3 (Layer C — curated versioned style-delegate contract) foundation delivered + verified 2026-08-24. Mechanism: PackRegistry scans `<packDir>/styles/*.qml` → `pack.styles`, `stylesCompatible(id, shellVer)` gates by minShellVersion (dotted semver compare) with a warning + base fallback on mismatch; shell.qml pushes the compatible id list onto `Appearance.activePackStyles`; `Appearance.styleUrl(id)` + `Appearance.shellVersion` ("0.3.0"); new Commons/Primitives/StyleDelegate.qml (Loader wrapper, source = styleUrl, `active` = loaded, passes a single forward-compatible `api` object). First curated component: GlassButton refactored so CHROME (shadow + rounded surface) is delegated while content/label/StateLayer/press-scale stay in the base (base visual hidden via `!styleDelegate.active`). HUD pack ships packs/hud/styles/GlassButton.qml (flat sharp dataslate frame + corner ticks, colours via api.colors — pack QML can't import Commons). docs/STYLE_API.md documents the seam + versioning + GlassButton api. Verified in isolation: base buttons render unchanged (no regression), forcing the HUD delegate swaps chrome to sharp+ticks with label/interaction intact, no QML errors. archeotech-shell commit 86bd84e. Set kept deliberately small per adr_027 ("small and last, expand deliberately") — additional components (DashCard, SegmentedControl, BarPill, PanelShadow, form Row) gain seams on demand when a pack needs them, not speculatively. Wave 4 (Layer D pack-scoped settings) remains.
